...
def generate_combined_dataset(motifs_to_test, repeats_map, flank_len, snp_rate, indel_rate, work_dir, replicate_id=0):
    """
    为单次重复模拟生成一个包含所有基元类型序列的组合FASTA文件和Truth文件。
    每个基元类型只生成一个序列 (num_sims=1)。
    """
            ...
            def evolve(sequence, target_list):
                for base in sequence:
                    ...
                    if rand_val < indel_rate:
                        ...
                        # else: pass # Deletion: simply do not append the base
...
def parse_trf_predictions(trf_dat_file):
    predictions_by_seq = defaultdict(list)
    if not os.path.exists(trf_dat_file) or os.path.getsize(trf_dat_file) == 0:
        return predictions_by_seq
    try:
        with open(trf_dat_file, 'r') as f:
            current_seq_id = ""
            for line in f:
                ...
                if line_stripped.startswith("Sequence:"):
                    current_seq_id = line_stripped.split(":", 1)[1].strip()
                    predictions_by_seq.setdefault(current_seq_id, [])
                    continue
                if line_stripped[0].isdigit():
                    ...
                    if len(parts) < 2:
                        continue
                    pred_start, pred_end = int(parts[0]), int(parts[1])
                    pred = {'start': pred_start, 'end': pred_end}
                    if len(parts) > 2:
                        try:
                            pred['period'] = int(float(parts[2]))
                        except ValueError:
                            pred['period'] = None
                    predictions_by_seq[current_seq_id].append(pred)
...
def evaluate_f1_by_motif(trf_dat_file, truth_file, motifs_to_test, match_mode="interval", overlap_threshold=0.5):
    """
    从组合文件中为每个基元类型计算F1分数。
    """
    ...
    predictions_by_seq = parse_trf_predictions(trf_dat_file)
...
# --- 模块3: 主控流程 ---
def main():
    parser = argparse.ArgumentParser(description="高级自动化TRF参数一体化网格搜索与统计优化流程 (重复模拟法, num_sims=1)。", formatter_class=argparse.RawTextHelpFormatter)
    parser.add_argument("--trf_path", required=True, help="TRF可执行文件的路径。")
    parser.add_argument("--num_replicates", type=int, default=30, help="为进行统计检验而执行的独立模拟重复次数。")
    parser.add_argument("--alignment_sets", default="2,7,7;2,5,5", help="测试的(Match,Mismatch,Delta)参数组合。")
    parser.add_argument("--pm_range", default="75,80", help="测试的PM(匹配概率)值范围。")
    parser.add_argument("--pi_range", default="10,20", help="测试的PI(插入/删除概率)值范围。")
    parser.add_argument("--score_range", default="30,40,50", help="测试的Score(最低对齐分数)值范围。")
    ...
    parser.add_argument("--output_csv", default="trf_grid_search_results.csv", help="保存网格搜索结果的CSV文件名。")
    parser.add_argument("--keep_files", action='store_true', help="保留所有中间生成的模拟和TRF结果文件。")
    ...
    parser.add_argument("--max_workers", type=int, default=1, help="并行运行TRF的最大并发数(每个replicate内)。")
    ...
    if not os.path.exists(args.trf_path):
        print(f"错误: TRF可执行文件未找到: {args.trf_path}"); return
    ...
    alignment_sets = []
    try:
        for s in args.alignment_sets.split(';'):
            parts = [p.strip() for p in s.split(',')]
            if len(parts) == 3: alignment_sets.append(tuple(int(p) for p in parts))
    except Exception as e:
        print(f"错误: 解析 --alignment_sets 参数失败: {e}"); return
    pm_values = [int(p) for p in args.pm_range.split(',')]
    pi_values = [int(p) for p in args.pi_range.split(',')]
    score_values = [int(s) for s in args.score_range.split(',')]
    ...
    flank_len = 200
    work_dir = "trf_grid_search_workspace"
    os.makedirs(work_dir, exist_ok=True)
    
    param_combos = [
        (m, mm, d, pm, pi, s)
        for m, mm, d in alignment_sets
        for pm in pm_values
        for pi in pi_values
    ...
    print("\n" + "="*50)
    print(f"--- 开始一体化网格搜索 (重复模拟法, num_sims=1) ---")
    ...
    if args.calibrate_threshold:
        ...
        for i in calibration_indices:
            ...
            def run_trf_for_calibration(j, params):
                match, mismatch, delta, pm, pi, score = params
                trf_dat_file = os.path.join(work_dir, f"trf_combo_{j}_rep_{i}.dat")

                if not os.path.exists(trf_dat_file):
                    trf_command = [
                        os.path.abspath(args.trf_path),
                        fasta_file_basename,
                        str(match), str(mismatch), str(delta),
                        str(pm), str(pi), str(score),
                        "6", "-d", "-h"
                    ]
                    with open(os.devnull, 'w') as devnull:
                        result = subprocess.run(trf_command, stdout=devnull, stderr=devnull, cwd=work_dir)
                    original_trf_out = os.path.join(
                        work_dir,
                        f"{fasta_file_basename}.{match}.{mismatch}.{delta}.{pm}.{pi}.{score}.6.dat"
                    )
                    if result.returncode != 0 and not os.path.exists(original_trf_out):
                        raise RuntimeError(
                            f"TRF运行失败: params={(match, mismatch, delta, pm, pi, score)}, returncode={result.returncode}"
                        )
                    if not os.path.exists(original_trf_out):
                        raise RuntimeError(f"TRF输出文件未生成: {original_trf_out}")
                    shutil.move(original_trf_out, trf_dat_file)
            ...
            if args.max_workers == 1:
                for j, params in enumerate(param_combos):
                    ...
                    try:
                        counts_by_threshold, null_ov = run_trf_for_calibration(j, params)
                    except Exception as e:
                        print(f"\n!!! 错误: {e}")
                        return
                ...
                with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
                    futures = {executor.submit(run_trf_for_calibration, j, params): params for j, params in enumerate(param_combos)}
    ...
    for idx, i in enumerate(evaluation_indices, start=1):
        ...
        def run_trf_and_eval(j, params):
            match, mismatch, delta, pm, pi, score = params
            param_tuple = (match, mismatch, delta, pm, pi, score)
            trf_dat_file = os.path.join(work_dir, f"trf_combo_{j}_rep_{i}.dat")
            ...
            if not os.path.exists(trf_dat_file):
                ...
                if result.returncode != 0 and not os.path.exists(original_trf_out):
                    raise RuntimeError(f"TRF运行失败: params={param_tuple}, returncode={result.returncode}")
                if not os.path.exists(original_trf_out):
                    raise RuntimeError(f"TRF输出文件未生成: {original_trf_out}")
                shutil.move(original_trf_out, trf_dat_file)
        ...
        if args.max_workers == 1:
            for j, params in enumerate(param_combos):
                print(f"\r    - Testing combo {j+1}/{len(param_combos)}...", end="")
                try:
                    param_tuple, f1_scores = run_trf_and_eval(j, params)
                except Exception as e:
                    print(f"\n!!! 错误: {e}")
                    return
            ...
            completed = 0
            with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
                futures = {executor.submit(run_trf_and_eval, j, params): params for j, params in enumerate(param_combos)}
                for future in as_completed(futures):
                    try:
    ...
    for param_tuple, f1_scores_per_replicate_list in results_dist.items():
        match, mismatch, delta, pm, pi, score = param_tuple
        ...
        # 创建一个包含所有结果的字典
        current_result = {
            'match': match, 'mismatch': mismatch, 'delta': delta, 'pm': pm, 'pi': pi, 'score': score,
    ...
    # 将平均值和CI放到最后
    display_cols = ['match', 'mismatch', 'delta', 'pm', 'pi', 'score'] + motif_f1_cols + ['f1_mean_avg', 'f1_mean_ci']
    ...
    if results_df.empty or results_df['f1_mean_avg'].sum() == 0:
        ...
        best_params_dict = {
            'match': int(best_row['match']), 'mismatch': int(best_row['mismatch']), 'delta': int(best_row['delta']),
            'pm': int(best_row['pm']), 'pi': int(best_row['pi']), 'score': int(best_row['score'])
        }
        ...
        print("\n" + "="*80)
        print("\n--- 最终结论: 推荐使用的最佳统一参数组合 (基于最高F1算术平均数) ---")
        print("  - 最佳参数组合:")
        for key, val in best_params_dict.items():
            print(f"    - {key.capitalize()}: {val}")
            
        mean_a_f1 = np.mean(best_a_mean_dist)
        ci_lower = np.percentile(best_a_mean_dist, 2.5)
        ci_upper = np.percentile(best_a_mean_dist, 97.5)
        print(f"  - 平均 F1 算术平均数: {mean_a_f1:.4f} (95% CI: [{ci_lower:.4f}, {ci_upper:.4f}])")

        # 修改：为最佳参数组合报告每个基元的平均F1分数（不带CI）
        print("\n  - 各基元类型的平均F1分数 (在最佳参数下):")
        f1_scores_by_motif = defaultdict(list)
        ...
        if len(results_df_sorted) > 1:
            ...
            print("\n--- 统计显著性分析 (配对检验) ---")
            sec_param_string = f"Match={int(second_best_row['match'])}, Mismatch={int(second_best_row['mismatch'])}, Delta={int(second_best_row['delta'])}, Pm={int(second_best_row['pm'])}, Pi={int(second_best_row['pi'])}, Score={int(second_best_row['score'])}"
    ...
    # 显式定义最终CSV的列顺序
    motif_f1_cols = [f'f1_{mtype}_avg' for mtype in motifs_to_test.keys()]
    final_cols_order = ['match', 'mismatch', 'delta', 'pm', 'pi', 'score'] + motif_f1_cols + ['f1_mean_avg', 'f1_mean_ci']
