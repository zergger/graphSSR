...
def parse_ultra_predictions(ultra_output_file):
    predictions_by_seq = defaultdict(list)
    if not os.path.exists(ultra_output_file) or os.path.getsize(ultra_output_file) == 0:
        return predictions_by_seq
    try:
        with open(ultra_output_file, 'r') as f:
            lines = f.readlines()

        header_index = -1
        for i, line in enumerate(lines):
            if line.strip().startswith('SeqID'):
                header_index = i
                break
        ...
        if header_index != -1:
            ...
            pred_df = pd.read_csv(csv_io, sep='\t')
        else:
            pred_df = None
    ...
    if pred_df is not None and not pred_df.empty:
        for row in pred_df.itertuples(index=False, name=None):
            ...
            seq_id = row[0]
            try:
                start = int(row[1]) + 1  # ULTRA start is 0-based
                end = int(row[2])
...
# --- 模块2: 评估器 (适配ULTRA输出) ---
def evaluate_f1_by_motif(ultra_output_file, truth_file, motifs_to_test, match_mode="interval", overlap_threshold=0.5):
    ...
    predictions_by_seq = parse_ultra_predictions(ultra_output_file)
...
# --- 模块3: 主控流程 ---
def main():
    parser = argparse.ArgumentParser(description="高级自动化ULTRA参数一体化网格搜索与统计优化流程 (重复模拟法)。", formatter_class=argparse.RawTextHelpFormatter)
    parser.add_argument("--num_replicates", type=int, default=30, help="为进行统计检验而执行的独立模拟重复次数。")
    # ULTRA特有的参数
    parser.add_argument("--match_range", default="0.7,0.8,0.85,0.9", help="测试的保守度(match)值，可以是单一数字或以逗号分隔的列表。")
    parser.add_argument("--decay_range", default="0.8,0.85,0.9,0.95", help="测试的衰减惩罚(decay)值，可以是单一数字或以逗号分隔的列表。")
    parser.add_argument("--ri_range", default="0.01,0.02,0.03", help="测试的插入概率(ri)值，可以是单一数字或以逗号分隔的列表。")
    parser.add_argument("--rd_range", default="0.01,0.02,0.03", help="测试的删除概率(rd)值，可以是单一数字或以逗号分隔的列表。")
    # 模拟参数
    ...
    parser.add_argument("--output_csv", default="ultra_grid_search_results.csv", help="保存网格搜索结果的CSV文件名。")
    parser.add_argument("--keep_files", action='store_true', help="保留所有中间生成的模拟和ULTRA结果文件。")
    ...
    parser.add_argument("--max_workers", type=int, default=1, help="并行运行ULTRA的最大并发数(每个replicate内)。")
    ...
    def parse_range(input_str):
        if ',' in input_str:
            return [float(p) for p in input_str.split(',')]
        else:
            return [float(input_str)]

    match_values = parse_range(args.match_range)
    decay_values = parse_range(args.decay_range)
    ri_values = parse_range(args.ri_range)
    rd_values = parse_range(args.rd_range)
    ...
    flank_len = 200
    work_dir = "ultra_grid_search_workspace"
    ...
    if args.calibrate_threshold:
        ...
        for i in calibration_indices:
            ...
            def run_ultra_for_calibration(j, params):
                match, decay, ri, rd = params
                ultra_output_file = os.path.join(work_dir, f"ultra_combo_{j}_rep_{i}.tsv")

                if not os.path.exists(ultra_output_file):
                    with open(ultra_output_file, 'w') as f_out:
                        ultra_command = [
                            'ultra',
                            fasta_file,
                            '--match', str(match),
                        ...
                        result = subprocess.run(ultra_command, check=False, stdout=f_out, stderr=subprocess.PIPE, text=True)
                    if result.returncode != 0:
                        stderr_text = (result.stderr or "").strip()
                        if len(stderr_text) > 500:
                            stderr_text = stderr_text[:500] + "..."
                        raise RuntimeError(
                            f"ULTRA运行失败: params={(match, decay, ri, rd)}, returncode={result.returncode}, stderr={stderr_text}"
                        )
                    if not os.path.exists(ultra_output_file):
                        raise RuntimeError(f"ULTRA输出文件未生成: {ultra_output_file}")
            ...
            if args.max_workers == 1:
                for j, params in enumerate(param_combos):
                    ...
                    try:
                        counts_by_threshold, null_ov = run_ultra_for_calibration(j, params)
                ...
                with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
                    futures = {executor.submit(run_ultra_for_calibration, j, params): params for j, params in enumerate(param_combos)}
    ...
    for idx, i in enumerate(evaluation_indices, start=1):
        ...
        def run_ultra_and_eval(j, params):
            match, decay, ri, rd = params
            param_tuple = (match, decay, ri, rd)
            ultra_output_file = os.path.join(work_dir, f"ultra_combo_{j}_rep_{i}.tsv")
            ...
            if not os.path.exists(ultra_output_file):
                ...
                if result.returncode != 0:
                    ...
                    if len(stderr_text) > 500:
                        stderr_text = stderr_text[:500] + "..."
                    raise RuntimeError(
                        f"ULTRA运行失败: params={param_tuple}, returncode={result.returncode}, stderr={stderr_text}"
                    )
                if not os.path.exists(ultra_output_file):
                    raise RuntimeError(f"ULTRA输出文件未生成: {ultra_output_file}")
        ...
        if args.max_workers == 1:
            for j, params in enumerate(param_combos):
                print(f"\r    - Testing combo {j+1}/{len(param_combos)}...", end="")
                try:
                    param_tuple, f1_scores = run_ultra_and_eval(j, params)
                except Exception as e:
                    print(f"\n!!! 错误: {e}")
                    return
            ...
            completed = 0
            with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
                futures = {executor.submit(run_ultra_and_eval, j, params): params for j, params in enumerate(param_combos)}
                for future in as_completed(futures):
                    try:
    ...
    for param_tuple, f1_scores_per_replicate_list in results_dist.items():
        ...
        current_result = {
            'match': match, 'decay': decay, 'ri': ri, 'rd': rd,
    ...
    print("\n" + "="*80)
    print("---                                 ULTRA网格搜索完整结果总结                                 ---")
    print("="*80)
    motif_f1_cols = [f'f1_{mtype}_avg' for mtype in motifs_to_test.keys()]
    display_cols = ['match', 'decay', 'ri', 'rd'] + motif_f1_cols + ['f1_mean_avg', 'f1_mean_ci']
    ...
    if results_df.empty or results_df['f1_mean_avg'].sum() == 0:
        ...
        if len(results_df_sorted) > 1:
            ...
            print("\n--- 统计显著性分析 (配对检验) ---")
            sec_param_string = f"--match={second_best_row['match']}, --decay={second_best_row['decay']}, --ri={second_best_row['ri']}, --rd={second_best_row['rd']}"
    ...
    motif_f1_cols = [f'f1_{mtype}_avg' for mtype in motifs_to_test.keys()]
    final_cols_order = ['match', 'decay', 'ri', 'rd'] + motif_f1_cols + ['f1_mean_avg', 'f1_mean_ci']
